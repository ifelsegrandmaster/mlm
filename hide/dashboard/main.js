...
function buildMatrix(){
   ...
   parent_node = {
      ...
      image: `http://${window.location.hostname}:8000/static/dashboard/images/user.png`,
      contact: "Tel: 0782 841 339"
   };
   ...
   if(first_child_data){
      first_child = {
         ...
         image: 'http://127.0.0.1:8000/static/dashboard/images/user.png'
...
function dashboardHandler(){
   ...
   request.open("GET", `http://${window.location.hostname}:8000/get_dashboard_data`)
   request.send(null)
...
function register(){
   ...
   request.open("GET", `http://localhost:8000/register/`)
   request.send(null)
}
...
function updateTable(data){
   ...
   for(var i = 0; i < data.length; i++){
      ...
      firstname.innerText = data[i].fields.first_name

      var lastname = document.createElement('td')
      lastname.innerText = data[i].fields.last_name

      var country = document.createElement('td')
      country.innerText = data[i].fields.country

      var phone = document.createElement('td')
      phone.innerText = data[i].fields.phone

      var level = document.createElement('td')
      level.innerText = data[i].fields.level
...
function initSelectors(){
   ...
   activity.addEventListener('change', function(){
      ...
      request.open("GET", `http://localhost:8000/search/activity/${activity.value}`)
      request.send(null)
   ...
   country.addEventListener('change', function(){
      ...
      request.open("GET", `http://localhost:8000/search/country/${country.value}`)
      request.send(null)
      ...
      if(data !== undefined)
        {
         ...
         for(var i = 0; i < data.length; i++){
            pdata = data[i]
             ...
             var action = document.createElement('td')
             if (type == 'completed')
                   action.innerHTML = `<a class='view-profile' href='/view/details/${pdata.id}'>View Profile</a>`;
             else action.innerHTML = `<a class='view-profile' href='/view/details/${pdata.id}'>View Profile</a>
             <button class='done' onclick='paymentDone(${pdata.pk})'>Paid</button>`;
...
function paymentDone(pk){
   ...
   request.open("GET", `http://localhost:8000/pay/${pk}`)
   request.send(null)
}
